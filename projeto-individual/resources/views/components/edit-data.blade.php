    ...
    <div class="w-full sm:max-w-md mt-6 px-6 py-4 bg-white shadow-md overflow-hidden sm:rounded-lg">
        ...
        <form method="POST" action="{{ route('pokemon.rename', $pokemon->id) }}" enctype="multipart/form-data">
            ...
            <div>
                ...
                <x-input id="name" class="block mt-1 w-full" type="text" name="name" :value="$pokemon->name" required autofocus />
            </div>
            ...
            <div class="mt-4 flex text-center">
                ...
                <div class="mr-4">
                    ...
                    <x-input id="defense" min="0" class="block mt-1 w-full" type="number" name="defense" :value="$pokemon->defense" />
                </div>

                <!-- Pokemon Healthy -->
                <div class="mr-4">
                    <x-label for="healthy" :value="__('Vida')" />
                    <x-input id="healthy" min="1" class="block mt-1 w-full" type="number" name="healthy" :value="$pokemon->healthy" />
                </div>
            ...
            <div class="mt-4 grid justify-items-center">
                ...
                <x-input id="stars" min="1" max="5" class="block mt-1 w-1/2" type="range" name="stars" :value="$pokemon->stars" oninput="this.nextElementSibling.value = this.value" /><output>{{ $pokemon->stars }}</output>
            </div>

            <!-- Image -->
            <div class="mt-4 text-center">
                <x-label for="image" :value="__('Imagem')" />
                <x-input id="image" class="border-2 border-slate-500 block mt-1 w-full bg-slate-400" type="file" name="image" :value="$pokemon->image" />
            </div>
